        ...
        <>
          ...
          <Dialog open={modal} onOpenChange={setModal}>
            <DialogContent className="w-[425px] max-h-dvh lg:max-w-screen-lg">
              <DialogHeader>
                <DialogTitle>{lang?.repl_balance}</DialogTitle>
              </DialogHeader>
              <DialogDescription>
                <div className='w-full mb-4'>
                  <Label className="text-xs text-muted-foreground mt-1">{lang?.amount}</Label>
                ...
                <Button onClick={() => {
                  setLoadingPay(true)
                  payCreate()
                }} disabled={loadingPay} className='w-full mt-2' variant='secondary'> {loadingPay? <div className="flex items-center gap-1"><Loader2 className="animate-spin w-4 h-4"/> {lang?.loading}</div> : lang?.pay_cryptocurrency}</Button>
              </DialogDescription>
            </DialogContent>
          </Dialog>
          <div className="pl-[260px] max-md:pl-[0px] transition-all pt-16 flex flex-col items-center">
            <div className="w-[1100px] max-2xl:w-full p-8 max-sm:p-4">
              ...
              {lang?.billing_desctiption}
              </p>

                <div className="flex gap-8 max-xl:flex-col">
                    <div className="flex flex-col w-[200px] max-xl:w-full">
                      <Link href="/billing/finances" className="h-10 pl-4 flex items-center hover:text-primary cursor-pointer bg-slate-100 font-medium border-l-2 border-primary">{lang?.finances}</Link>
                      <Link href="/billing/subscriptions" className="h-10 pl-4 flex items-center hover:text-primary cursor-pointer">{lang?.subscriptions}</Link>
                    </div>
                    <div className="flex flex-col w-[calc(100%_-_232px)] max-xl:w-full">
                        ...
                        <div className="border rounded-md flex flex-col mt-4">
                            <div className="p-8 max-lg:p-4">
                                <b>{lang?.invoices}</b>
                                <p className="mt-2 mb-4">{lang?.invoices_desc}</p>
                            </div>

                            <div className="flex flex-col w-full">
                              <div className="flex w-full border-b bg-slate-100 max-lg:hidden">
                                <div className="w-[25%] pt-2 pb-2 pr-4 font-semibold text-sm pl-4">{lang?.date}</div>
                                <div className="w-[25%] pt-2 pb-2 pr-4 font-semibold text-sm pl-4">{lang?.payer_currency}</div>
                                <div className="w-[20%] pt-2 pb-2 pr-4 font-semibold text-sm pl-4">{lang?.amount}</div>
                                <div className="w-[15%] pt-2 pb-2 pr-4 font-semibold text-sm pl-4">{lang?.status}</div>
                                <div className="w-[15%] pt-2 pb-2 pr-4 font-semibold text-sm pl-4 flex justify-end text-sm">{lang?.payment}</div>
                              </div>
                              {pay?.map((el:any) => 
                                <div key={el.id} className="w-full flex border-b max-lg:hidden">
                                  ...
                                  <div className="w-[15%] pb-2 pt-2 pl-4 pr-4 text-smflex justify-end text-sm flex "><Link href={`https://pay.cryptomus.com/pay/${el.payment_id}`} className="cursor-pointer flex text-primary items-center">{lang?.to_pay} <ChevronRight className={"w-4 transition-all h-[16px]"}/></Link></div>
                                </div>
                              )}
                              ...
                              {pay?.map((el:any) => 
                                <div key={el.id} className="w-full p-4 hidden max-lg:flex flex-col border-t">
                                  <div className="flex items-center w-full">
                                    <div className="w-[40%] max-sm:w-[50%] text-sm font-bold">{lang?.date}</div>
                                  ...
                                  <div className="flex pb-2 pt-2 items-center w-full">
                                    <div className="w-[40%] max-sm:w-[50%] text-sm font-bold">{lang?.payer_currency}</div>
                                    <div className="text-sm">{el.payer_currency == null ? "NULL" : el.payer_currency}</div>
                                  </div>
                                  <div className="flex pb-2 pt-2 items-center w-full">
                                    <div className="w-[40%] max-sm:w-[50%] text-sm font-bold">{lang?.amount}</div>
                                    <div className="text-sm">{Number(el.amount).toFixed(2)} $</div>
                                  </div>
                                  <div className="flex pb-2 pt-2 items-center w-full">
                                    <div className="w-[40%] max-sm:w-[50%] text-sm font-bold">{lang?.status}</div>
                                    <div className="text-sm">{el?.status}</div>
                                  </div>
                                  <div className="flex pb-2 pt-2 items-center w-full">
                                    <div className="w-[40%] max-sm:w-[50%] text-sm font-bold">{lang?.payment}</div>
                                    <div><Link href={`https://pay.cryptomus.com/pay/${el.payment_id}`} className="cursor-pointer flex text-primary items-center text-sm">{lang?.to_pay} <ChevronRight className={"w-4 transition-all h-[16px]"}/></Link></div>
                                  </div>
                                </div>
                              )}
                            </div>
